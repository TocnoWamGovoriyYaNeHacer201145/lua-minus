...
show("testing numbers and math lib")
...
global<const> show, assert, pcall, type, pairs, load
...
do
  ...
  show(string.format(msg, intbits, floatbits, maxexp))
end
...
if floatbits < intbits then
  show("testing order (floats cannot represent all integers)")
  ...
  show("testing order (floats can represent all integers)")
  assert(maxint < maxint + 1.0)
...
do   -- testing floor & ceil
  ...
  checkerror("number expected", math.ceil, show)
...
do
  ...
  show(string.format("random seeds: %d, %d", x, y))
end
...
do   -- test random for floats
  ...
  show(string.format("float random range in %d calls: [%f, %f]",
                      totalrounds, low, up))
end
...
--
-- [[==================================================================
    show("testing precision of 'tostring'")
-- -===================================================================
--

-- number of decimal digits supported by float precision
local decdig = math.floor(floatbits * math.log(2, 10))
show(string.format("  %d-digit float numbers with full precision",
                    decdig))
-- number of decimal digits supported by integer precision
local Idecdig = math.floor(math.log(maxint, 10))
show(string.format("  %d-digit integer numbers with full precision",
                    Idecdig))

do
  -- Any number should show so that reading it back gives itself:
  ...
  -- different numbers should show differently.
  ...
  -- "reasonable" numerals should be showed like themselves
