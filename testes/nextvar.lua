...
show('testing tables, next, and for')
...
local function showTable (t)
  ...
  show("array:")
  for i = 1, a do
    show("", T.querytab(t, i - 1))
  end
  show("hash:")
  for i = 1, h do
    show("", T.querytab(t, a + i - 1))
  end
end
----------------------------------------------------------------
...
do  show("testing attack on table length")
  local t = {}
...
assert(show==find("show") and show == find1("show"))
assert(_G["show"]==find("show"))
...
assert(table.maxn{[10] = true, [100*math.pi] = show} == 100*math.pi)
...
do
  show("testing next x GC of deleted keys")
  -- bug in 5.4.1
...
do
  show("testing floats in numeric for")
  local a
  -- integer count
