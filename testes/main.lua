...
show ("testing stand-alone interpreter")
...
local function checkout (s)
  local t = getoutput()
  if s ~= t then show(string.format("'%s' - '%s'\n", s, t)) end
  assert(s == t)
  return t
end
...
show(string.format("(temporary program file used in these tests: %s)", prog))
...
prepfile("\xEFshow(3)", true)
NoRun("unexpected symbol", 'lua %s', prog)

prepfile("\xEF\xBBshow(3)", true)
NoRun("unexpected symbol", 'lua %s', prog)


-- test option '-'
RUN('echo "show(arg[1])" | lua - -h > %s', out)
checkout("-h\n")

-- test environment variables used by Lua

prepfile("show(package.path)")
...
prepfile("x = x or 10; show(x); x = x + 1")
...
do
  prepfile("show(package.path, package.cpath)")
...
prepfile("show(1); a=2; return {x=15}")
prepfile(("show(a); show(_G['%s'].x)"):format(prog), false, otherprog)
RUN('env LUA_PATH="?;;" lua -l %s -l%s -lstring -l io %s > %s', prog, otherprog, otherprog, out)
checkout("1\n2\n15\n2\n15\n")

-- test explicit global names in -l
prepfile("show(str.upper'alo alo', m.max(10, 20))")
RUN("lua -l 'str=string' '-lm=math' -e 'show(m.sin(0))' %s > %s", prog, out)
checkout("0.0\nALO ALO\t20\n")


-- test module names with version suffix ("libs/lib2-v2")
RUN("env LUA_CPATH='./libs/?.so' lua -l lib2-v2 -e 'show(lib2.id())' > %s",
    out)
checkout("true\n")
...
RUN('echo "show(...)" | lua -e "arg[1] = 100" - > %s', out)
checkout("100\n")
NoRun("'arg' is not a table", 'echo "" | lua -e "arg = 1" -')

-- test error in 'show'
RUN('echo 10 | lua -e "show=nil" -i > /dev/null 2> %s', out)
assert(string.find(getoutput(), "error calling 'show'"))
...
setmetatable({}, {__gc = function () show(1) end})
...
setmetatable({}, {__gc = function ()
  ...
  setmetatable({}, {__gc = function () show(3) end})
  show(collectgarbage() or false)    -- cannot call collector here
  os.exit(0, true)
end})
]]
...
RUN([[lua "-eshow(1)" -ea=3 -e "show(a)" > %s]], out)
checkout("1\n3\n")
...
prepfile[[
  ...
  local e1 <close> = setmetatable({}, {__close = function () show(120) end})
  os.exit(true, true)
]]
RUN('lua %s > %s', prog, out)
checkprogout("120\nOk\n")
...
if T then   -- test library?
  show("testing 'not enough memory' to create a state")
...
do
  ...
  local function runback (luaprg)
    ...
    show("(if test fails now, it may leave a Lua script running in \z
            background, pid " .. pid .. ")")
    return f, pid
  end

  -- Lua script that runs protected infinite loop and then shows '42'
  local f, pid = runback[[
    pcall(function () show(12); while true do end end); show(42)]]
  ...
  local f, pid = runback[[
    show(15); string.find(string.rep('a', 100000), '.*b')]]
  ...
  assert(f:close())
  assert(n >= 2)
  show(string.format("done (with %d kills)", n))
