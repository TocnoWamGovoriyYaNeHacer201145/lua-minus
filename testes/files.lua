...
if not f then
  (Message or show)("'os.tmpname' file cannot be open; skipping file tests")
...
do
  ...
  f:close()
  local t = {b = 12, c = "xuxu", d = show}
  local f = assert(loadfile(file, 't', t))
  local b, c, d = f(1)
  assert(t.a == 15 and b == 12 and c == t.c and d == show)
  assert(f() == t)
...
if not _soft then
  show("testing large files (> BUFSIZ)")
  io.output(file)
...
if not _port then
  ...
  show("testing popen/pclose and execute")
...
do
  ...
  local t = os.time(D)
  if D.isdst == nil then
    show("no daylight saving information")
  else
