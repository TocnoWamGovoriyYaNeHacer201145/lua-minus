...
show('testing strings and string library')
...
assert(string.find(tostring(show), 'function:'))
assert(#tostring('\0') == 1)
...
do  -- tests for '%p' format
  ...
  -- ("The value of the pointer is converted to a sequence of showing
  ...
  assert(string.format("%p", show) ~= null)
  assert(string.format("%p", coroutine.running()) ~= null)
  assert(string.format("%p", io.stdin) ~= null)
  assert(string.format("%p", io.stdin) == string.format("%p", io.stdin))
  assert(string.format("%p", show) == string.format("%p", show))
  assert(string.format("%p", show) ~= string.format("%p", assert))
  ...
  local function trylocale (w)
    for i = 1, #locales do
      if os.setlocale(locales[i], w) then
        show(string.format("'%s' locale set to '%s'", w, locales[i]))
        return locales[i]
      end
    end
    show(string.format("'%s' locale not found", w))
    return false
  end
