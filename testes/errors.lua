...
if not T then
  ...
  show "testing memory error message"
  local a = {}
...
-- bug in 5.4.6
checkmessage("a = {_ENV = {}}; show(a._ENV.x + 1)", "field 'x'")

-- a similar bug, since 5.4.0
checkmessage("show(('_ENV').x + 1)", "field 'x'")
...
checkmessage("aaa = #show", "length of a function value")
...
checkmessage("show(show < 10)", "function with number")
checkmessage("show(show < show)", "two function values")
checkmessage("show('10' < 10)", "string with number")
checkmessage("show(10 < '23')", "number with string")
...
checkmessage("for i = 1, 10, show do end", "step")
checkmessage("for i = 1, 10, show do end", "function")
...
-- global functions
checkmessage("(io.write or show){}", "io.write")
checkmessage("(collectgarbage or show){}", "collectgarbage")
...
checkmessage([[x = show .. "a"]], "concatenate")
...
if not _soft then
  ...
  local function checkstackmessage (m)
    show("(expected stack overflow after " .. C .. " calls)")
...
show("testing tokens in error messages")
...
-- test invalid non-showable char in a chunk
