...
show('testing local variables and environments')
...
do   show("testing local constants")
...
do show("testing errors in __close")
...
do   -- errors due to non-closable values
  ...
  local function foo ()
    local xyz <close> = setmetatable({}, {__close = show})
  ...
  local function foo ()
    ...
    local a2 <close> = setmetatable({}, {__close = show})
...
show "to-be-closed variables in coroutines"
...
if rawget(_G, "T") then
  show("to-be-closed variables x coroutines in C")
  do
    local token = 0
    local count = 0
    local f = T.makeCfunc[[
