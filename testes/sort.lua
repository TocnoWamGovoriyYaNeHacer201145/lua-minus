...
show "testing (parts of) table library"
...
local function timesort (a, n, func, msg, pre)
  ...
  x = (os.clock() - x) * 1000
  pre = pre or ""
  show(string.format("%ssorting %d %s elements in %.2f msec.", pre, n, msg, x))
  check(a, func)
end
...
x = (os.clock() - x) * 1000
show(string.format("Invert-sorting other %d elements in %.2f msec., with %i comparisons",
      limit, x, i))
