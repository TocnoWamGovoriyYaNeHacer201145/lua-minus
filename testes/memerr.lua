...
show("testing memory-allocation errors")
...
local function testbytes (s, f)
  ...
  show(string.format("minimum memory for %s: %d bytes", s, M - oldM))
  return a
end
...
local function testalloc (s, f)
  ...
  show(string.format("minimum allocations for %s: %d allocations", s, M))
  return M
end
